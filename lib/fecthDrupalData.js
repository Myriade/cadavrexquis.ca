...
export const useFetchAllFilmsData = (initialData) => {
	const [data, setData] = useState(null)
	const [isLoading, setIsLoading] = useState(true)
	const [error, setError] = useState(null)
	const [cacheIsUsable, setCacheIsUsable] = useState(null)
		
	const filmsCacheKey = 'allFilmsCache'
	...
	async function fetchDrupalFilms() {
		try {	
			...
			localStorage.setItem(filmsCacheKey, jsonData);
			localStorage.setItem('cacheTimestamp', Date.now())
			
		} catch (error) {
			console.error('Myriade Web - useFetchAllFilmsData : Erreur lors du chargement des données', error);
	...
	useEffect(() => {
		if (typeof window !== 'undefined' && isLoading && !data && cacheIsUsable === null) {
			const resultBool = cacheCheck(filmsCacheKey, 'cacheTimestamp')
			setCacheIsUsable(resultBool)
		}
	}, [cacheIsUsable, data, isLoading])
	
	// set of fetch and set data
	useEffect( () => {
		// set data with usable cached data
		if (!data && cacheIsUsable === true && isLoading) {
			const cachedData = JSON.parse(localStorage.getItem(filmsCacheKey))
		...
		if (!data && cacheIsUsable === false && isLoading) {
			console.log('Fetching data from Drupal DB')
			fetchDrupalFilms()
		}
