...
export const useFetchAllFilms = (isSearch) => {
	...
	const searchQueryParams = {fields: {
			"node--film": "path,drupal_internal__nid,title,field_annees_de_sortie,field_site_thematique,field_realisation,field_site_collection,field_site_visible,field_vedettes_matiere,field_site_photogramme,field_descriptions_cadavrexquis,field_resume_de_l_institution_de",
		},
		"filter[field_site_visible]": 1,
...
// Fetch unique film node in Drupal DB from its unique identifier
...
// Fetch PAGE node from its node ID in Drupal DB
