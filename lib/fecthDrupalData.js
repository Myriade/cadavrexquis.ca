...
///// ---- DOCUMENTS -----/////
// Charger tous documents pour la grille. 
export const useFetchAllDocuments = () => {
	...
	const cacheValueKey = 'allDocuments'
	const cacheTimeKey = 'allDocumentsTimestamp'
			...
			async function fetchDrupalDocuments() {
				try {	
					...
					const response = await drupal.getResourceCollection("node--article", {
						...
						params: {
							fields: {
								"node--article": 'path,drupal_internal__nid,title,body'
							},
							"filter[status]": 1,
							"page[limit]": 200
						}
	...
	}, [docData, cacheIsUsable, docIsLoading, cacheTimeKey, cacheValueKey])
