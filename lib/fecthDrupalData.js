...
// Cache check
async function cacheCheck(dataKey, timestampKey) {	
	// const cacheTimestamp = localStorage.getItem(timestampKey);
	const cacheTimestamp = await localforage.getItem(timestampKey);
	...
	const storedData = await localforage.getItem(dataKey)
	const parsedDataObj = await JSON.parse(storedData)
	const cachedDataIsEmpty = await isEmpty(parsedDataObj)
	...
	useEffect(() => {
		async function verifyCache () {
			if (typeof window !== 'undefined' && isLoading && !data && cacheIsUsable === null) {
				const resultBool = await cacheCheck(cacheValueKey, cacheTimeKey)
	...
	useEffect( () => {
		async function setCachedData () {
			if (!data && cacheIsUsable === true && isLoading) {
				...
				const bruteCachedData = await localforage.getItem(cacheValueKey);
				const cachedData = await JSON.parse(bruteCachedData);
	...
	useEffect( () => {
		async function cacheData () {
			if (!data && filmData && docData && cacheIsUsable === false && isLoading) {
				...
				await localforage.setItem(cacheValueKey, jsonData);
				await localforage.setItem(cacheTimeKey, Date.now());
			}
		}
		cacheData();
...
const useFetchFilmsPaths = () => {
	...
	async function fetchFilmsPaths() {
		try {	
			...
			await localforage.setItem(pathsCacheValueKey, jsonData);
			await localforage.setItem(pathsCacheTimeKey, Date.now())
	...
	useEffect(() => {
		async function verifyCache () {
			if (typeof window !== 'undefined' && isLoading && !paths && cacheIsUsable === null) {
				const resultBool = await cacheCheck(pathsCacheValueKey, pathsCacheTimeKey)
				setCacheIsUsable(resultBool)
			}
		}
		verifyCache()
	...
	useEffect( () => {
		...
		async function setOrFetchPaths () {
			if (!paths && cacheIsUsable === true && isLoading) {
				...
				const bruteCachedData = await localforage.getItem(pathsCacheValueKey)
				const cachedData = await JSON.parse(bruteCachedData);
				setPaths(cachedData)
			}
		}
...
const useFetchDocumentsPaths = () => {
	...
	useEffect( () => {
		...
		async function setCachedData () {
			if (!paths && cacheIsUsable === true && isLoading) {
				// const cachedData = JSON.parse(localStorage.getItem(pathsCacheValueKey));
				const cachedData = await localforage.getItem(pathsCacheValueKey)
