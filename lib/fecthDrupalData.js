...
export const useFetchFilmsAndDocuments = (isSearch) => {
	...
	const docBasicQueryParams = {
		fields: {
			"node--article": 'path,drupal_internal__nid,field_site_visible,title,field_site_thematique,field_site_photogramme,field_annees_de_sortie'
		},
		"filter[field_site_visible]": 1,
		"include": 'field_site_thematique,field_site_photogramme',
		"page[limit]": 200
	...
	useEffect( () => {
		...
		if (!data && filmData && docData && isLoading) {
			let result = filmData;
	...
	}, [data, filmData, docData, cacheIsUsable, isLoading, filmsQueryParams, docsQueryParams, cacheTimeKey, cacheValueKey])
